...
# Define the file_id for each video in different qualities
videos = {
    'amateur': {
...
async def quality_handler(update: Update, context):
    ...
    # Check if the user previously selected a level
    if chosen_level:
        quality_map = {
            'quality_480p': '480p',
            'quality_720p': '720p',
        ...
        # Выбираем видео на основе уровня и качества
        ...
        await query.message.reply_video(
            video=video_id,  # Use the file_id corresponding to the chosen quality
...
# Мэйн Фанкшн, который запускает бота и хэндлеры
def main():
